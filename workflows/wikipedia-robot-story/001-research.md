...
Use `agent-browser` to navigate to Wikipedia and search for "robot":

```bash
agent-browser open https://en.wikipedia.org
```
...
2. Use `agent-browser eval` or snapshots to extract the key content from the page — focus on:
